...
import com.example.firebaserecyclerviewkotlin.MainActivityMedic
...
class FragmentLogin : Fragment(R.layout.fragment_login) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        userLoginBtn.setOnClickListener{
            ...
            if(email.isEmpty())  {
                Toast.makeText(this@FragmentLogin.requireActivity(), "Enter email", Toast.LENGTH_SHORT).show()
                return@setOnClickListener
            }

            if(password.isEmpty()) {
                Toast.makeText(this@FragmentLogin.requireActivity(), "Enter password", Toast.LENGTH_SHORT).show()
                return@setOnClickListener
            }
            ...
            FirebaseAuth.getInstance()
                ...
                .addOnCompleteListener { task ->
                    if (task.isSuccessful){
                        val intent = Intent (this@FragmentLogin.requireContext(), MainActivityMedic::class.java)
                        startActivity(intent)

                    }
                    else {
                        Toast.makeText(this@FragmentLogin.requireActivity(), "Couldn't sign in, account not found", Toast.LENGTH_SHORT).show()
                    }
                }
        }
    }}
